...
# Agentic University System (Level 6)
try:
    from app.api.v1.endpoints import universities
    app.include_router(universities.router, prefix="/api/v1", tags=["Universities"])
    logger.info("✅ University endpoints enabled (Level 6)")
except ImportError as e:
    logger.warning(f"University endpoints not available: {e}")

try:
    from app.api.v1.endpoints import curriculum
    app.include_router(curriculum.router, prefix="/api/v1", tags=["Curriculum"])
    logger.info("✅ Curriculum endpoints enabled (Level 6)")
except ImportError as e:
    logger.warning(f"Curriculum endpoints not available: {e}")

try:
    from app.api.v1.endpoints import competitions
    app.include_router(competitions.router, prefix="/api/v1", tags=["Competitions"])
    logger.info("✅ Competition endpoints enabled (Level 6)")
except ImportError as e:
    logger.warning(f"Competition endpoints not available: {e}")

...
class ToolRegistryService:
    ...
    async def register_tool(self, tool_data: ToolCatalogCreate) -> AgentToolCatalog:
        ...
        try:
            ...
            existing = self.db.query(AgentToolCatalog).filter(
    ...
    async def get_tool(self, tool_id: str) -> Optional[AgentToolCatalog]:
        """Get a tool by ID."""
        return self.db.query(AgentToolCatalog).filter(
            AgentToolCatalog.tool_id == tool_id
        ).first()
    ...
    async def list_tools(
        ...
        query = self.db.query(AgentToolCatalog)

        if enabled_only:
            query = query.filter(AgentToolCatalog.enabled == True)

        if category:
            query = query.filter(AgentToolCatalog.category == category)

        if security_level:
            query = query.filter(AgentToolCatalog.security_level == security_level)

        return query.order_by(AgentToolCatalog.name).all()
    ...
    async def validate_tool_request(
        ...
        # Check permissions
        permission = self.db.query(ToolPermission).filter(
            and_(
...
def get_default_tool_catalog() -> List[Dict[str, Any]]:
    ...
    async def create_stateful_session(
        ...
        try:
            ...
            # Get tool
            tool = self.db.query(AgentToolCatalog).filter(
                AgentToolCatalog.tool_id == tool_id
            ).first()

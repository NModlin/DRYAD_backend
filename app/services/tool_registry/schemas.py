...
class ToolCreate(BaseModel):
    ...
    schema_json: dict[str, Any] = Field(..., description="OpenAPI 3.0 specification")
    ...
    def validate_openapi_schema(cls, v: dict[str, Any]) -> dict[str, Any]:
        """Validate that schema_json contains required OpenAPI 3.0 fields."""
        if not isinstance(v, dict):
            raise ValueError("schema_json must be a dictionary")
        
        # Check for required OpenAPI 3.0 fields
        if "openapi" not in v:
            raise ValueError("schema_json must contain 'openapi' field")
        
        if "info" not in v:
            raise ValueError("schema_json must contain 'info' field")
        
        if "paths" not in v:
            raise ValueError("schema_json must contain 'paths' field")

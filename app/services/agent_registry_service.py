...
class AgentRegistryService:
    ...
    def register_agent(
        ...
        try:
            ...
            if tenant_id:
                # SQLite-compatible JSON query
                existing_count = self.db.query(SystemAgent).filter(
            ...
            existing = self.db.query(SystemAgent).filter(
                SystemAgent.agent_id == agent_data.agent_id
            ).first()
    ...
    def get_agent(
        ...
        """
        query = self.db.query(SystemAgent).filter(SystemAgent.agent_id == agent_id)
    ...
    def list_agents(
        ...
        # Apply filters
        if tier:
            query = query.filter(SystemAgent.tier == tier)
        if status:
            query = query.filter(SystemAgent.status == status)
        if category:
            query = query.filter(SystemAgent.category == category)
        if tenant_id:
            # SQLite-compatible JSON query
            query = query.filter(
                SystemAgent.configuration.like(f'%"tenant_id": "{tenant_id}"%')
        ...
        agents = query.order_by(SystemAgent.created_at.desc()).all()
    ...
    def discover_agents_by_capabilities(
        ...
        """
        query = self.db.query(SystemAgent).filter(SystemAgent.status == AgentStatus.ACTIVE)
        ...
        if exclude_agents:
            query = query.filter(~SystemAgent.agent_id.in_(exclude_agents))
    ...
    def get_capability_registry(self, tenant_id: Optional[str] = None) -> Dict[str, List[str]]:
        """
    ...
    def get_orchestration_patterns(self, tenant_id: Optional[str] = None) -> Dict[str, List[str]]:
        """
    ...
    async def update_metrics(self, metrics_update: AgentMetricsUpdate) -> bool:
        ...
        if not agent:
            logger.warning(f"Agent not found for metrics update: {metrics_update.agent_id}")
            return False
        ...
        await self.db.commit()
        logger.debug(f"Updated metrics for agent: {metrics_update.agent_id}")
        return True

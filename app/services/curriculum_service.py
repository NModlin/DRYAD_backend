...
class CurriculumService:
    ...
    @staticmethod
    def create_curriculum_path(db: Session, path_data: CurriculumPathCreate) -> CurriculumPath:
        """
        ...
        db.add(curriculum_path)
        db.commit()
        db.refresh(curriculum_path)
        
        return curriculum_path

    @staticmethod
    def get_curriculum_path(db: Session, path_id: str) -> Optional[CurriculumPath]:
        """Get a curriculum path by ID"""
        return db.query(CurriculumPath).filter(CurriculumPath.id == path_id).first()

    @staticmethod
    def get_curriculum_paths_by_university(
        db: Session,
        university_id: str,
        ...
        query = db.query(CurriculumPath).filter(CurriculumPath.university_id == university_id)
        ...
        return query.order_by(CurriculumPath.created_at.desc()).offset(skip).limit(limit).all()
    ...
    def update_curriculum_path(
        ...
        curriculum_path = db.query(CurriculumPath).filter(CurriculumPath.id == path_id).first()
    ...
    @staticmethod
    def delete_curriculum_path(db: Session, path_id: str) -> bool:
    ...
    @staticmethod
    def create_curriculum_level(db: Session, level_data: CurriculumLevelCreate) -> CurriculumLevel:
        ...
        db.add(curriculum_level)
        db.commit()
        db.refresh(curriculum_level)
        
        return curriculum_level

    @staticmethod
    def get_curriculum_level(db: Session, level_id: str) -> Optional[CurriculumLevel]:
        """Get a curriculum level by ID"""
        return db.query(CurriculumLevel).filter(CurriculumLevel.id == level_id).first()

    @staticmethod
    def get_levels_by_path(db: Session, path_id: str) -> List[CurriculumLevel]:
        """Get all levels for a curriculum path, ordered by level number"""
        return db.query(CurriculumLevel).filter(
            CurriculumLevel.curriculum_path_id == path_id
        ).order_by(CurriculumLevel.level_number).all()

    @staticmethod
    def delete_curriculum_level(db: Session, level_id: str) -> bool:
        """Delete a curriculum level"""
        curriculum_level = db.query(CurriculumLevel).filter(CurriculumLevel.id == level_id).first()
        
        if not curriculum_level:
            return False
        
        db.delete(curriculum_level)
        db.commit()
    ...
    @staticmethod
    def enroll_agent(db: Session, enrollment_data: AgentProgressCreate) -> AgentCurriculumProgress:
        """
        ...
        existing = db.query(AgentCurriculumProgress).filter(
            and_(
        ...
        curriculum_path = db.query(CurriculumPath).filter(
            CurriculumPath.id == enrollment_data.curriculum_path_id
        ).first()
        if curriculum_path:
    ...
    @staticmethod
    def get_agent_progress(db: Session, agent_id: str, path_id: str) -> Optional[AgentCurriculumProgress]:
        """Get agent's progress on a curriculum path"""
        return db.query(AgentCurriculumProgress).filter(
            and_(
    ...
    def get_agent_all_progress(
        ...
        query = db.query(AgentCurriculumProgress).filter(AgentCurriculumProgress.agent_id == agent_id)
    ...
    @staticmethod
    def start_curriculum(db: Session, agent_id: str, path_id: str) -> Optional[AgentCurriculumProgress]:
    ...
    def complete_level(
        ...
        curriculum_path = db.query(CurriculumPath).filter(CurriculumPath.id == path_id).first()
        if curriculum_path:
            total_levels = db.query(func.count(CurriculumLevel.id)).filter(
                CurriculumLevel.curriculum_path_id == path_id
            ).scalar()
            ...
            if len(progress.completed_levels) >= total_levels:
                ...
                if curriculum_path.total_completions > 0:
                    all_progress = db.query(AgentCurriculumProgress).filter(
                        and_(
                            AgentCurriculumProgress.curriculum_path_id == path_id,
                ...
                all_scores = db.query(AgentCurriculumProgress.overall_score).filter(
                    and_(
                        AgentCurriculumProgress.curriculum_path_id == path_id,
                ...
                if all_scores:
                    curriculum_path.average_score = sum(s[0] for s in all_scores) / len(all_scores)

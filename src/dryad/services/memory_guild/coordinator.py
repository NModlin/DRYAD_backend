...
from dryad.services.memory_guild.models import (
...
class MemoryCoordinatorAgent:
    ...
    def __init__(self, db: AsyncSession, enable_specialists: bool = True):
        self.db = db
        ...
        if enable_specialists:
            try:
                from dryad.services.memory_guild.archivist import ArchivistAgent
            ...
            try:
                from dryad.services.memory_guild.librarian import LibrarianAgent
    ...
    async def _retrieve_database(
        ...
        try:
            ...
            if request.key:
                stmt = stmt.where(MemoryRecord.memory_metadata['key'] == request.key)
            elif request.memory_id:
                stmt = stmt.where(MemoryRecord.memory_id == uuid.UUID(request.memory_id))
            ...
            return MemoryResponse(
                ...
                data={
                    ...
                    "key": memory_record.memory_metadata.get("key") if memory_record.memory_metadata else None,
                    "value": {"content": memory_record.content_text},
                    "metadata": memory_record.memory_metadata.get("additional_metadata", {}) if memory_record.memory_metadata else {}
                },
                source="database"
            )
    ...
    async def _delete_database(
        ...
        try:
            ...
            # First find the record(s) to confirm existence/count
            stmt = select(MemoryRecord).where(
                MemoryRecord.tenant_id == (request.tenant_id or "default")
            )
            ...
            # Now delete them
            delete_stmt = delete(MemoryRecord).where(
                MemoryRecord.memory_id.in_([r.memory_id for r in records])
            )
    ...
    async def _get_memory_policy(self, tenant_id: str) -> MemoryPolicy:
        ...
        try:
            ...
            stmt = select(MemoryAccessPolicy).where(MemoryAccessPolicy.tenant_id == tenant_id)
            result = await self.db.execute(stmt)
            ...
            if policy_record:
                policy = MemoryPolicy(
                    ...
                    short_term_ttl=3600, # Default for now
                    long_term_enabled=True,
                    max_memory_size=1000000,
                    retention_days=30,
                    access_rules=policy_record.policy_rules or {}
                )
                self.active_policies[tenant_id] = policy
                return policy
                
        except Exception as e:
            logger.log_warning("policy_load_failed", {"tenant_id": tenant_id, "error": str(e)})

        # Return default policy if not found or error
        default_policy = MemoryPolicy(tenant_id=tenant_id)
        self.active_policies[tenant_id] = default_policy

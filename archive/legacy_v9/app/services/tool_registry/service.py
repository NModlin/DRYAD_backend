...
from app.database.models.tool_registry import ToolRegistry, ToolPermission, ToolExecution, ToolSession, ToolStatus, SecurityLevel
...
class ToolRegistryService:
    ...
    async def register_tool(self, tool_data: ToolCreate) -> ToolResponse:
        ...
        if not isinstance(tool_data.configuration_schema, dict):
            raise InvalidToolSchemaError("Schema must be a dictionary")
        
        if "openapi" not in tool_data.configuration_schema:
        ...
        tool = ToolRegistry(
            ...
            function_signature=tool_data.configuration_schema,
            permissions={},
            requires_sandbox=False,
            max_session_duration=300,
            stateful=False,
            active=True,
            sandbox_image=tool_data.docker_image_uri
        )
        ...
        try:
            ...
            return ToolResponse(
                ...
                configuration_schema=tool.function_signature,
                docker_image_uri=tool.sandbox_image,
    ...
    async def get_tool_by_id(self, tool_id: str | UUID) -> ToolResponse:
        """
        Get a specific tool by its ID.

        Args:
            tool_id: Tool ID (UUID or string)
        ...
        Raises:
            ToolNotFoundError: If tool not found
        """
        stmt = select(ToolRegistry).where(ToolRegistry.id == str(tool_id))
        result = await self.db.execute(stmt)
        tool = result.scalar_one_or_none()
    ...
    async def update_tool(
        ...
        for field, value in update_dict.items():
            if field == "is_active":
                 tool.active = value
            elif field == "docker_image_uri":
                 tool.sandbox_image = value
            elif hasattr(tool, field):
    ...
    async def validate_tool_request(
        ...
        Args:
            tool_id: ID of the tool (or name)
            agent_category: Category/Role of the agent
            agent_tier: Optional tier of the agent
        ...
        # If not found by ID, try by name (latest version)
        if not tool:
            stmt = select(ToolRegistry).where(ToolRegistry.name == tool_id).order_by(ToolRegistry.created_at.desc())
            result = await self.db.execute(stmt)
        ...
        if not tool.active:
            return {
                "allowed": False,
                "reason": f"Tool '{tool.name}' is inactive",
                "constraints": {}
            }
        ...
        # Check permissions
        # Look for permission granted to the specific agent category (role)
        stmt = select(ToolPermission).where(
            and_(
                ToolPermission.tool_id == tool.id,
                ToolPermission.principal_type == "role",
                ToolPermission.principal_id == agent_category
            )
        )
        result = await self.db.execute(stmt)
        ...
        if not permission:
            if tool.security_level in [SecurityLevel.SAFE, SecurityLevel.LOW_RISK]:
                return {
                    "allowed": True,
                    "reason": "Tool is safe for general use",
                    "constraints": {
                        "max_execution_time": tool.max_session_duration,
                        "requires_approval": tool.requires_sandbox # Using sandbox requirement as proxy for approval/safety check
                    }
                }
            else:
                return {
        ...
        # If permission active/revoked check could be here, but model doesn't have explicit 'allowed' bool, 
        # existence implies allowed unless we add deny logic. Assuming existence = allow for now.
        
        # Build constraints from permission conditions if any
        constraints = {
            "max_execution_time": tool.max_session_duration,
            "requires_approval": tool.requires_sandbox
        }
        
        if permission.conditions:
            constraints.update(permission.conditions)
    ...
    async def get_tool_history(
        ...
        count_stmt = select(func.count()).select_from(ToolExecution).where(ToolExecution.tool_id == str(tool_id))
        total_result = await self.db.execute(count_stmt)
        total = total_result.scalar()
        
        # Get paginated executions
        exec_stmt = select(ToolExecution).where(ToolExecution.tool_id == str(tool_id)).order_by(ToolExecution.created_at.desc()).limit(limit).offset(offset)

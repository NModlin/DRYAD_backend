...
class GuardrailService:
    ...
    async def _ensure_configs_loaded(self):
        """Load active guardrail configurations if not loaded."""
        if self._configs_loaded:
            return

        result = await self.db.execute(
            select(GuardrailConfiguration).where(GuardrailConfiguration.enabled == True)
        )
    ...
    async def get_violation_history(
        ...
        """
        try:
            stmt = select(ExecutionGuardrail)

            if agent_id:
                stmt = stmt.where(ExecutionGuardrail.agent_id == agent_id)

            if guardrail_type:
                stmt = stmt.where(ExecutionGuardrail.guardrail_type == guardrail_type)

            if severity:
                stmt = stmt.where(ExecutionGuardrail.severity == severity)

            result = await self.db.execute(
                stmt.order_by(ExecutionGuardrail.timestamp.desc()).limit(limit)
            )
    ...
    async def get_metrics_summary(
        ...
        """
        try:
            stmt = select(GuardrailMetrics)

            if start_date:
                stmt = stmt.where(GuardrailMetrics.date >= start_date.date())

            if end_date:
                stmt = stmt.where(GuardrailMetrics.date <= end_date.date())

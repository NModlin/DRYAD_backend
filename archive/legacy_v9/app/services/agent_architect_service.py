...
class AgentArchitectService:
    ...
    async def continue_conversation(
        ...
        try:
            ...
            if self._is_complete(conversation_id):
                ...
                validation_result = await self.validator.validate_agent_sheet(agent_sheet)
    ...
    def _generate_agent_sheet(self, conversation_id: str) -> Dict[str, Any]:
        ...
        agent_sheet = {
            "agent_sheet_version": "1.0.0",
            "metadata": {
                "submitted_by": state.get("user_id", "anonymous"),
                "submission_date": datetime.utcnow().isoformat(),
                "client_name": "Agent Studio",
                "purpose": gathered["goal"]
            },
            "agent_definition": {
                "name": agent_name,
                "display_name": gathered["role"].title(),
                "description": gathered["goal"],
                "version": "1.0.0",
            ...
            "capabilities": {
                ...
                "skills": gathered.get("capabilities", [])
            },
            "behavior": {
                 "temperature": 0.7,
                 "max_tokens": 2048,
            ...
            "integration": {
                 "dryad_compatible": True,
                 "oracle_integration": True
            },
            "validation": {
                "test_queries": ["Hello, who are you?"]
            },
            "tools": [{"name": t, "description": "Auto-added", "enabled": True} for t in gathered.get("tools", [])]
        }

...
from sqlalchemy import Column, String, Integer, Float, Boolean, DateTime, JSON, Text, Enum as SQLEnum, ForeignKey
...
class SystemAgent(Base):
    ...
    preferred_collaborators = Column(JSON, nullable=True, default=list)  # List of agent_ids for peer collaboration
    ...
    # Configuration
    configuration = Column(JSON, nullable=False, default=dict)  # Agent-specific config
    llm_config = Column(JSON, nullable=True)  # LLM preferences (temperature, max_tokens, etc.)
    tools = Column(JSON, nullable=False, default=list)  # Available tools
    ...
    # Additional data
    extra_data = Column(JSON, nullable=True)  # Additional metadata (renamed from 'metadata' to avoid SQLAlchemy conflict)
